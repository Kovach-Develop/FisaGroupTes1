...
public class Trains {
    ...
    private static String parseRouteLength(int distance){
        return distance == -1? "No hay ruta " : Integer.toString(distance);
    }

    public static void main(String[] args) throws IOException {
        if (args.length == 0) {
            System.err.println ("Error, Necesitas un parámetro de ruta de archivo");
            System.exit(0);
        }
        String graphStr = new String(Files.readAllBytes(Paths.get(args[0])));
        GraphService graph = new GraphService(graphStr);
